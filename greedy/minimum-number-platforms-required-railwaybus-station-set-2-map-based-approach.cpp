...
int findPlatform(int arr[], int dep[], int n)
{
	...
	for (int i = 0; i < n; i++) {
		...
		// of pair is 'a' else 'd'
		order.insert(make_pair(arr[i], 'a'));       // here we need to add one extra condition that if there are two timing with exact
                                                    // same time then it will fail so we need to add count with it as well
